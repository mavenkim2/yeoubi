...
void CUDADevice::BuildBVH(Scene *scene)
{
    ...
    for (Curves &curve : scene->curves)
    {
        ...
        BuildOptixBVH(this, *deviceArena, options, input);
    ...
    YBI_ASSERT(gridClusterTemplateHandles.size());
    ...
    for (MicropolygonMesh &mesh : scene->micropolygonMeshes)
    {
        ...
        DeviceMemoryView<float3> deviceGridVertices =
            deviceArena->PushArray<float3>(numPositions);
        ...
        for (size_t gridIdx = 0; gridIdx < mesh.grids.size(); gridIdx++)
        {
            ...
            const OptixTraversableHandle templateHandle =
                gridClusterTemplateHandles[templateIndex];
        ...
        DeviceMemoryView<OptixClusterAccelBuildInputTemplatesArgs> deviceTemplateArgs =
            deviceArena->PushArray<OptixClusterAccelBuildInputTemplatesArgs>(
                numBuildGrids);
        CUDA_ASSERT(
        ...
        DeviceMemoryView<uint32_t> deviceArgsCount =
            deviceArena->PushArray<uint32_t>(1);
        ...
        OPTIX_ASSERT(optixClusterAccelComputeMemoryUsage(optixDeviceContext,
        ...
        DeviceMemoryView<uint32_t> deviceOutputSizes =
            deviceArena->PushArray<uint32_t>(numBuildGrids);
        DeviceMemoryView<uint8_t> temp = deviceArena->PushArray<uint8_t>(
        ...
        DeviceMemoryView<uint8_t> explicitOutput = Alloc<uint8_t>(totalOutputSize);
        ...
        DeviceMemoryView<uint64_t> deviceOutputOffsets =
            deviceArena->PushArray<uint64_t>(numBuildGrids);
