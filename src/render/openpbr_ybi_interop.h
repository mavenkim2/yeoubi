...
template <typename T>
YBI_DEVICE constexpr T openpbr_ybi_min(const T a, const T b)
{
    return a < b ? a : b;
}

template <typename T>
YBI_DEVICE constexpr T openpbr_ybi_max(const T a, const T b)
{
    return a > b ? a : b;
}

YBI_DEVICE float openpbr_ybi_abs(const float x)
{
    return fabsf(x);
}

YBI_DEVICE vec2 openpbr_ybi_abs(const vec2 v)
{
    return vec2(fabsf(v.x), fabsf(v.y));
}

YBI_DEVICE vec3 openpbr_ybi_abs(const vec3 v)
{
    return vec3(fabsf(v.x), fabsf(v.y), fabsf(v.z));
}

YBI_DEVICE vec4 openpbr_ybi_abs(const vec4 v)
{
    return vec4(fabsf(v.x), fabsf(v.y), fabsf(v.z), fabsf(v.w));
}

YBI_DEVICE float openpbr_ybi_acos(const float x)
{
    return acosf(x);
}

YBI_DEVICE float openpbr_ybi_atan(const float x)
{
    return atanf(x);
}

YBI_DEVICE float openpbr_ybi_atan(const float y, const float x)
{
    return atan2f(y, x);
}

YBI_DEVICE float openpbr_ybi_cos(const float x)
{
    return cosf(x);
}

YBI_DEVICE float openpbr_ybi_exp(const float x)
{
    return expf(x);
}

YBI_DEVICE vec3 openpbr_ybi_exp(const vec3 v)
{
    return vec3(expf(v.x), expf(v.y), expf(v.z));
}

YBI_DEVICE float openpbr_ybi_floor(const float x)
{
    return floorf(x);
}

YBI_DEVICE float openpbr_ybi_log(const float x)
{
    return logf(x);
}

YBI_DEVICE vec3 openpbr_ybi_log(const vec3 v)
{
    return vec3(logf(v.x), logf(v.y), logf(v.z));
}

YBI_DEVICE float openpbr_ybi_pow(const float x, const float y)
{
    return powf(x, y);
}

YBI_DEVICE vec3 openpbr_ybi_pow(const vec3 a, const vec3 b)
{
    return vec3(powf(a.x, b.x), powf(a.y, b.y), powf(a.z, b.z));
}

YBI_DEVICE vec3 openpbr_ybi_pow(const vec3 a, const float b)
{
    return vec3(powf(a.x, b), powf(a.y, b), powf(a.z, b));
}

YBI_DEVICE float openpbr_ybi_sin(const float x)
{
    return sinf(x);
}

YBI_DEVICE float openpbr_ybi_sqrt(const float x)
{
    return sqrtf(x);
}

YBI_DEVICE vec3 openpbr_ybi_sqrt(const vec3 v)
{
    return vec3(sqrtf(v.x), sqrtf(v.y), sqrtf(v.z));
}

YBI_DEVICE vec2 openpbr_ybi_min(const vec2 a, const vec2 b)
{
    return vec2(openpbr_ybi_min(a.x, b.x), openpbr_ybi_min(a.y, b.y));
}

YBI_DEVICE vec3 openpbr_ybi_min(const vec3 a, const vec3 b)
{
    return vec3(openpbr_ybi_min(a.x, b.x), openpbr_ybi_min(a.y, b.y), openpbr_ybi_min(a.z, b.z));
}

YBI_DEVICE vec4 openpbr_ybi_min(const vec4 a, const vec4 b)
{
...
YBI_DEVICE vec2 openpbr_ybi_max(const vec2 a, const vec2 b)
{
    return vec2(openpbr_ybi_max(a.x, b.x), openpbr_ybi_max(a.y, b.y));
}

YBI_DEVICE vec3 openpbr_ybi_max(const vec3 a, const vec3 b)
{
    return vec3(openpbr_ybi_max(a.x, b.x), openpbr_ybi_max(a.y, b.y), openpbr_ybi_max(a.z, b.z));
}

YBI_DEVICE vec4 openpbr_ybi_max(const vec4 a, const vec4 b)
{
...
YBI_DEVICE float openpbr_ybi_clamp(const float x, const float lo, const float hi)
{
    return openpbr_ybi_max(lo, openpbr_ybi_min(x, hi));
}

YBI_DEVICE vec2 openpbr_ybi_clamp(const vec2 v, const vec2 lo, const vec2 hi)
{
    return openpbr_ybi_max(lo, openpbr_ybi_min(v, hi));
}

YBI_DEVICE vec3 openpbr_ybi_clamp(const vec3 v, const vec3 lo, const vec3 hi)
{
    return openpbr_ybi_max(lo, openpbr_ybi_min(v, hi));
}

YBI_DEVICE vec4 openpbr_ybi_clamp(const vec4 v, const vec4 lo, const vec4 hi)
{
    return openpbr_ybi_max(lo, openpbr_ybi_min(v, hi));
}

YBI_DEVICE vec2 openpbr_ybi_clamp(const vec2 v, const float lo, const float hi)
{
    return openpbr_ybi_clamp(v, vec2(lo), vec2(hi));
}

YBI_DEVICE vec3 openpbr_ybi_clamp(const vec3 v, const float lo, const float hi)
{
    return openpbr_ybi_clamp(v, vec3(lo), vec3(hi));
}

YBI_DEVICE vec4 openpbr_ybi_clamp(const vec4 v, const float lo, const float hi)
{
    return openpbr_ybi_clamp(v, vec4(lo), vec4(hi));
}

YBI_DEVICE float openpbr_ybi_mix(const float a, const float b, const float t)
{
    return a * (1.0f - t) + b * t;
}

YBI_DEVICE vec2 openpbr_ybi_mix(const vec2 a, const vec2 b, const float t)
{
    return a * (1.0f - t) + b * t;
}

YBI_DEVICE vec3 openpbr_ybi_mix(const vec3 a, const vec3 b, const float t)
{
    return a * (1.0f - t) + b * t;
}

YBI_DEVICE vec4 openpbr_ybi_mix(const vec4 a, const vec4 b, const float t)
{
    return a * (1.0f - t) + b * t;
}

YBI_DEVICE vec2 openpbr_ybi_mix(const vec2 a, const vec2 b, const vec2 t)
{
    return vec2(openpbr_ybi_mix(a.x, b.x, t.x), openpbr_ybi_mix(a.y, b.y, t.y));
}

YBI_DEVICE vec3 openpbr_ybi_mix(const vec3 a, const vec3 b, const vec3 t)
{
...
YBI_DEVICE float openpbr_ybi_smoothstep(const float edge0, const float edge1, const float x)
{
...
YBI_DEVICE float openpbr_ybi_dot(const vec2 a, const vec2 b)
{
    return ybi::Dot(a, b);
}

YBI_DEVICE float openpbr_ybi_dot(const vec3 a, const vec3 b)
{
    return ybi::Dot(a, b);
}

YBI_DEVICE float openpbr_ybi_dot(const vec4 a, const vec4 b)
{
    return ybi::Dot(a, b);
}

YBI_DEVICE vec3 openpbr_ybi_cross(const vec3 a, const vec3 b)
{
    return ybi::Cross(a, b);
}

YBI_DEVICE float openpbr_ybi_length(const vec2 v)
{
    return ybi::Length(v);
}

YBI_DEVICE float openpbr_ybi_length(const vec3 v)
{
    return ybi::Length(v);
}

YBI_DEVICE float openpbr_ybi_length(const vec4 v)
{
    return ybi::Length(v);
}

YBI_DEVICE vec3 openpbr_ybi_normalize(const vec3 v)
{
    return ybi::Normalize(v);
}

YBI_DEVICE vec3 openpbr_ybi_reflect(const vec3 incident, const vec3 normal)
{
    return ybi::Reflect(incident, normal);
}

YBI_DEVICE vec2 openpbr_ybi_equal(const vec2 a, const vec2 b)
{
    return vec2(a.x == b.x ? 1.0f : 0.0f, a.y == b.y ? 1.0f : 0.0f);
}

YBI_DEVICE vec3 openpbr_ybi_equal(const vec3 a, const vec3 b)
{
    return vec3(a.x == b.x ? 1.0f : 0.0f, a.y == b.y ? 1.0f : 0.0f, a.z == b.z ? 1.0f : 0.0f);
}

YBI_DEVICE vec2 openpbr_ybi_greaterThan(const vec2 a, const vec2 b)
{
    return vec2(a.x > b.x ? 1.0f : 0.0f, a.y > b.y ? 1.0f : 0.0f);
}

YBI_DEVICE vec3 openpbr_ybi_greaterThan(const vec3 a, const vec3 b)
{
    return vec3(a.x > b.x ? 1.0f : 0.0f, a.y > b.y ? 1.0f : 0.0f, a.z > b.z ? 1.0f : 0.0f);
}

YBI_DEVICE vec3 openpbr_ybi_greaterThanEqual(const vec3 a, const vec3 b)
{
    return vec3(a.x >= b.x ? 1.0f : 0.0f, a.y >= b.y ? 1.0f : 0.0f, a.z >= b.z ? 1.0f : 0.0f);
}

YBI_DEVICE vec2 openpbr_ybi_notEqual(const vec2 a, const vec2 b)
{
    return vec2(a.x != b.x ? 1.0f : 0.0f, a.y != b.y ? 1.0f : 0.0f);
}

YBI_DEVICE vec3 openpbr_ybi_notEqual(const vec3 a, const vec3 b)
{
    return vec3(a.x != b.x ? 1.0f : 0.0f, a.y != b.y ? 1.0f : 0.0f, a.z != b.z ? 1.0f : 0.0f);
}

YBI_DEVICE vec4 openpbr_ybi_notEqual(const vec4 a, const vec4 b)
{
...
YBI_DEVICE bool openpbr_ybi_all(const vec2 v)
{
    return v.x != 0.0f && v.y != 0.0f;
}

YBI_DEVICE bool openpbr_ybi_all(const vec3 v)
{
    return v.x != 0.0f && v.y != 0.0f && v.z != 0.0f;
}

YBI_DEVICE bool openpbr_ybi_any(const vec2 v)
{
    return v.x != 0.0f || v.y != 0.0f;
}

YBI_DEVICE bool openpbr_ybi_any(const vec3 v)
{
    return v.x != 0.0f || v.y != 0.0f || v.z != 0.0f;
}

YBI_DEVICE bool openpbr_ybi_any(const vec4 v)
{
    return v.x != 0.0f || v.y != 0.0f || v.z != 0.0f || v.w != 0.0f;
}

YBI_DEVICE float openpbr_ybi_saturate(const float x)
{
    return openpbr_ybi_clamp(x, 0.0f, 1.0f);
}

YBI_DEVICE vec3 openpbr_ybi_saturate(const vec3 v)
{
    return openpbr_ybi_clamp(v, 0.0f, 1.0f);
}

YBI_DEVICE vec2 openpbr_swizzle_xy(const vec3 v)
{
    return vec2(v.x, v.y);
}

YBI_DEVICE vec2 openpbr_swizzle_xy(const vec4 v)
{
    return vec2(v.x, v.y);
}

YBI_DEVICE vec3 openpbr_swizzle_xyz(const vec4 v)
{
